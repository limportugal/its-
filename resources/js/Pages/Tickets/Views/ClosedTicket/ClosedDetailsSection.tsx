...
import { Box, Paper, Table, TableBody, TableCell, TableContainer, TableRow, Typography, Button } from "@mui/material";
...
const ClosedDetailsSection: React.FC<ClosedDetailsSectionProps> = ({
    ...
    return (
        <Box>
            <Box
                sx={{
                    background: `linear-gradient(135deg, ${alpha(getStatusColors('closed').text, 0.1)} 0%, ${alpha(getStatusColors('closed').text, 0.05)} 100%)`,
                    borderRadius: '12px 12px 0 0',
                    p: 2,
                    border: `1px solid ${alpha(getStatusColors('closed').text, 0.2)}`,
                    borderBottom: 'none',
                    position: 'relative',
                    overflow: 'hidden',
                    '&::before': {
                        ...
                        right: 0,
                        height: '3px',
                        background: `linear-gradient(90deg, ${getStatusColors('closed').text} 0%, ${alpha(getStatusColors('closed').text, 0.7)} 100%)`,
                    }
                }}
            >
                <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
                    <Typography
                        variant="subtitle1"
                        sx={{
                            color: getStatusColors('closed').text,
                            fontWeight: 700,
