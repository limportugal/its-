import { useState, useCallback, useEffect } from "react";
import { useQueryClient } from "@tanstack/react-query";
import { useController } from "react-hook-form";

// MUI COMPONENTS
import CustomDialog from "@/Components/Mui/CustomDialog";
import SnackBarAlert from "@/Components/Mui/SnackBarAlert";
import { CircularProgress, Box, Grid, useMediaQuery, useTheme, Button, Typography } from "@mui/material";
import ClearButton from "@/Components/Mui/ClearButton";

// HOOKS, API, TYPE, HELPERS, UTILS & VALIDATIONS
import { useDynamicMutation } from "@/Reuseable/hooks/useDynamicMutation";
import { CancelTicketProps } from "@/Reuseable/types/ticketTypes";
import { reopenTicketData } from "@/Reuseable/api/ticket/reopen-ticket.api";

// REACT HOOK FORM WITH ZOD VALIDATION
import { useForm } from "react-hook-form";
import MuiTextField from "@/Components/Mui/MuiTextField";
import { zodResolver } from "@hookform/resolvers/zod";
import { router } from "@inertiajs/react";
import { ReOpenTicketValues, reOpenTicketSchema } from "@/Reuseable/validations/reOpenTicketValidation";
import { snakeCaseToTitleCase } from "@/Reuseable/utils/capitalize";
import StatusChip from "@/Pages/Tickets/TicketComponents/StatusChip";

// CREATE CATEGORY COMPONENT
const ReOpenTicket: React.FC<CancelTicketProps> = ({ open, onClose, onCancel, ticket }) => {
    const theme = useTheme();
    const isMobile = useMediaQuery(theme.breakpoints.down("sm"));
    const [showSnackBarAlert, setShowSnackBarAlert] = useState(false);
    const [snackbarMessage, setSnackbarMessage] = useState("");
    const queryClient = useQueryClient();

    // FORM INITIAL VALUES
    const defaultValues = {
        re_open_reason: "",
    };

    // ZOD RESOLVER FOR VALIDATION
    const {
        control,
        handleSubmit,
        reset,
        setError,
        formState: { errors },
    } = useForm<ReOpenTicketValues>({
        defaultValues,
        mode: "all",
        shouldFocusError: true,
        resolver: zodResolver(reOpenTicketSchema),
    });

    const { field } = useController({
        name: "re_open_reason",
        control,
    });

    // CLEAR FORM
    const resetForm = () => {
        reset(defaultValues);
    };

    // RESET FORM WHEN THE DIALOG CLOSES
    useEffect(() => {
        if (open) {
            resetForm();
        }
    }, [open]);

    // HANDLE DIALOG CLOSE
    const handleClose = () => {
        resetForm();
        onClose();
    };


    // FETCH USERS DATA USING USEQUERY HOOK
    const {
        mutate: reopenTicketMutation,
        isPending: isReopenTicketLoading,
    } = useDynamicMutation({
        mutationFn: (params: { uuid: string, re_open_reason: string }) => {
            const result = reopenTicketData(params.uuid, params.re_open_reason);
            return result;
        },
        mutationKey: "getPendingTickets",
        onSuccess: async () => {
            await Promise.all([
                queryClient.invalidateQueries({
                    queryKey: ["getPendingTickets"],
                }),
                queryClient.invalidateQueries({
                    queryKey: ["getClosedTickets"],
                }),
                queryClient.invalidateQueries({
                    queryKey: ["getCancelledTickets"],
                }),
                queryClient.invalidateQueries({
                    queryKey: ["getViewPendingTicketData"],
                }),
            ]);
            
            setShowSnackBarAlert(true);
            setSnackbarMessage("Successfully reopened ticket.");
            reset({ re_open_reason: "" });
            onClose();

            // REFRESH THE PAGE
            setTimeout(() => {
                router.visit(route('tickets.indexPendingTickets'));
            }, 1000);
        },
        onError: (error: any) => {
            let errorMessage = "An error occurred while reopening the ticket. Please try again.";
            
            // Handle timeout errors specifically
            if (error?.message?.includes('timeout')) {
                errorMessage = "The request is taking longer than expected. Please check if the ticket was reopened successfully.";
            } else if (error?.response?.data?.message) {
                errorMessage = error.response.data.message;
            }
            
            setError("re_open_reason", {
                type: "manual",
                message: errorMessage
            });
        },
    });

    // MEMOIZE HANDLE FORM SUBMISSION
    const submitForm = useCallback(async (data: ReOpenTicketValues) => {
        if (!ticket?.uuid) {
            return;
        }
        
        reopenTicketMutation({
            uuid: ticket.uuid,
            re_open_reason: data.re_open_reason
        });
    }, [ticket?.uuid, reopenTicketMutation]);

    // RENDER THE FORM
    return (
        <>
            <CustomDialog
                open={open}
                onClose={handleClose}
                maxWidth="sm"
                fullWidth
                isLoading={isReopenTicketLoading}
                title={
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <Typography variant="h6">Re-open Ticket</Typography>
                        <StatusChip
                            label={snakeCaseToTitleCase(ticket?.status || '')}
                            status={ticket?.status}
                        />
                    </Box>
                }
            >
                <form onSubmit={handleSubmit(submitForm)}>
                    {/* TEXTFIELD */}
                    <Grid
                        container
                        sx={{
                            px: 2,
                            pt: 1,
                        }}
                    >
                        <Grid size={{ xs: 12 }}>
                            <MuiTextField
                                {...field}
                                control={control}
                                label="RE-OPEN REASON"
                                placeholder="Enter your reason for reopening this ticket."
                                fullWidth
                                multiline
                                maxRows={10}
                                minRows={2}
                                rows={Math.min(Math.max((field.value?.split('\n').length || 2), 2), 10)}
                                errors={errors}
                                disabled={isReopenTicketLoading}
                            />
                        </Grid>

                        {/* BUTTONS */}
                        <Grid
                            size={{ xs: 12 }}
                            sx={{
                                display: "flex",
                                justifyContent: "flex-end",
                                gap: 1,
                                my: isMobile ? 1 : 2
                            }}
                        >
                            <ClearButton
                                minWidth="150px"
                                fullWidth={isMobile ? true : false}
                                disabled={isReopenTicketLoading}
                                onClick={() => reset({ re_open_reason: "" })}
                            />
                            <Button
                                variant="contained"
                                color="primary"
                                onClick={handleSubmit(submitForm)}
                                disabled={isReopenTicketLoading}
                                fullWidth={isMobile ? true : false}
                                sx={{ minWidth: "150px", position: "relative" }}
                            >
                                {isReopenTicketLoading ? (
                                    <>
                                        REOPENING...
                                        <CircularProgress
                                            size={20}
                                            color="primary"
                                            sx={{ marginLeft: 1 }}
                                        />
                                    </>
                                ) : (
                                    "REOPEN TICKET"
                                )}
                            </Button>
                        </Grid>
                    </Grid>
                </form>
            </CustomDialog >

            {/* SNACKBAR ALERTS */}
            {
                showSnackBarAlert && (
                    <SnackBarAlert
                        open={true}
                        severity="success"
                        message={snackbarMessage}
                        onClose={() => setShowSnackBarAlert(false)}
                        anchorOrigin={{ vertical: "top", horizontal: "right" }}
                    />
                )
            }
        </>
    );
};

export default ReOpenTicket;
