...
import { deleteSystemData } from "@/Reuseable/api/maintenance/system.api";

export const deleteSystemHandler = async (row: any, refetch: () => void) => {
    const confirmed = await showDeleteConfirmationAlert(
        undefined,
        `Do you really want to delete this <span style="color: #1565c0;">"${row.system_name}"</span>?<br/> 
    ...
    if (confirmed) {
        try {
            // DELETE THE SYSTEM
            await deleteSystemData(row.id);

            await showDeletedSuccessAlert(
                undefined,
                `<span style="color: #1565c0;">"${row.system_name}"</span> has been deleted successfully.`,
            );
            refetch();
        } catch (error) {
            await showDeleteErrorAlert(
                undefined,
                `<span style="color: #1565c0;">"${row.system_name}"</span> could not be deleted. 
