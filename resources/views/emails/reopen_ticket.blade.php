...
<head>
    ...
    <style>
        ...
        @media only screen and (max-width: 600px) {
            ...
            /* Mobile styles for unified reopen card */
            .unified-reopen-card {
                padding: 20px;
                margin: 16px 0;
            }
        ...
        .reopen-details-card::before {
            ...
            right: 0;
            bottom: 0;
            background: url('data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><defs><pattern id="grain" width="100" height="100" patternUnits="userSpaceOnUse"><circle cx="25" cy="25" r="1" fill="%230ea5e9" opacity="0.03"/><circle cx="75" cy="75" r="1" fill="%230ea5e9" opacity="0.03"/><circle cx="50" cy="10" r="0.5" fill="%230ea5e9" opacity="0.02"/><circle cx="10" cy="60" r="0.5" fill="%230ea5e9" opacity="0.02"/><circle cx="90" cy="40" r="0.5" fill="%230ea5e9" opacity="0.02"/></pattern></defs><rect width="100" height="100" fill="url(%23grain)"/></svg>');
            pointer-events: none;
        }

        .reopen-details-content {
            position: relative;
            z-index: 1;
        }
...
<body>
    <div class="container">
        <div class="header">
            <h1>Hi, {{ $full_name }}!</h1>
            <p>Your ticket has been reopened and is now back in our system.</p>
        ...
        <!-- UNIFIED REOPEN DETAILS CARD WITH AGENT AVATAR -->
        <div class="unified-reopen-card">
            <div class="unified-reopen-content">
                <!-- AGENT AVATAR SECTION -->
                <div class="agent-section">
                    @if(isset($reopened_user_avatar_public) && $reopened_user_avatar_public)
                        <img src="{{ $reopened_user_avatar_public }}" alt="{{ $reopened_user_first_letter ?? 'A' }}" class="unified-agent-avatar">
                    @elseif(isset($reopened_user_avatar_url) && $reopened_user_avatar_url)
                        <img src="{{ $reopened_user_avatar_url }}" alt="{{ $reopened_user_first_letter ?? 'A' }}" class="unified-agent-avatar">
                    @elseif(isset($reopened_user_avatar_base64) && $reopened_user_avatar_base64)
                        <img src="{{ $reopened_user_avatar_base64 }}" alt="{{ $reopened_user_first_letter ?? 'A' }}" class="unified-agent-avatar">
                    @else
                    ...
                    <div class="agent-info">
                        <div class="agent-label">Reopened By Agent</div>
                        <h3 class="agent-name">{{ $reopened_by ?? $name }}</h3>
                        @if(isset($reopenedAt))
                        <div class="agent-timestamp">
                            Reopened on {{ date('M d, Y \a\t h:i A', strtotime($reopenedAt)) }}
                        </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="reopen-reason-section">
                    <div class="reopen-label">Reopen Details</div>
                    <div class="reopen-reason">{{ $reopenReason }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="message">
            ...
            Your ticket has been reopened and is now back in our support queue. We'll continue working on resolving your issue.
        </div>
